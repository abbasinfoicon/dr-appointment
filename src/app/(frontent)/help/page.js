import React from 'react'
import Banner from '@/app/components/Banner';

const Help = () => {
    return (
        <>
            <Banner title="Help" img="bnr1.jpg" />

            <div className="section-full content-inner">
                <div className="container">
                    <div className="row">
                        <div className="col-lg-9">
                            <h1 className="m-b15 m-t0">Health Care</h1>
                            <p><strong>Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's
                                standard dummy text ever since the  when an unknown printer took a galley of type and scrambled it to make a
                                type specimen book. </strong></p>
                            <h2 className="m-t30 m-b10 ">Emergency Service</h2>
                            <p>Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy
                                text ever since the  when an unknown printer took a galley of type and scrambled it to make a type specimen book. Lorem Ipsum is
                                simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the
                                when an unknown printer took a galley of type and scrambled it to make a type specimen book. </p>
                            <div className="row">
                                <div className="col-md-6 col-sm-6 col-12">
                                    <div className="icon-bx-wraper bx-style-1 p-a20 left m-b30">
                                        <div className="bg-primary icon-bx-xs m-b20 "> <span className="icon-cell text-white"> <i className="fa fa-shopping-basket"></i> </span> </div>
                                        <div className="icon-content">
                                            <h5 className="dez-tilte m-b5">Our Best Doctor</h5>
                                            <p>Lorem ipsum dolor sit adipiscing sed diam nonummy end </p>
                                        </div>
                                    </div>
                                </div>
                                <div className="col-md-6 col-sm-6 col-12">
                                    <div className="icon-bx-wraper bx-style-1 p-a20 left m-b30">
                                        <div className="bg-primary icon-bx-xs m-b20"> <span className="icon-cell text-white"> <i className="fa fa-truck"></i> </span> </div>
                                        <div className="icon-content">
                                            <h5 className="dez-tilte m-b5">Fast Ambulance Searvice</h5>
                                            <p>Lorem ipsum dolor sit adipiscing sed diam nonummy end </p>
                                        </div>
                                    </div>
                                </div>
                                <div className="col-md-6 col-sm-6 col-12">
                                    <div className="icon-bx-wraper bx-style-1 p-a20 left m-b30">
                                        <div className="bg-primary icon-bx-xs m-b20"> <span className="icon-cell text-white"><i className="fa fa-map-marker"></i> </span> </div>
                                        <div className="icon-content">
                                            <h5 className="dez-tilte m-b5">Well Qualified Staff</h5>
                                            <p>Lorem ipsum dolor sit adipiscing sed diam nonummy end </p>
                                        </div>
                                    </div>
                                </div>
                                <div className="col-md-6 col-sm-6 col-12">
                                    <div className="icon-bx-wraper bx-style-1 p-a20 left m-b30">
                                        <div className="bg-primary icon-bx-xs m-b20 "> <span className="icon-cell text-white"> <i className="fa fa-user"></i> </span> </div>
                                        <div className="icon-content">
                                            <h5 className="dez-tilte m-b5">24X7 Service</h5>
                                            <p>Lorem ipsum dolor sit adipiscing sed diam nonummy end </p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <p>Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy
                                text ever since the  when an unknown printer took a galley of type and scrambled it to make a type specimen book. </p>
                            <h3 className="m-b10">Tips </h3>
                            <ul className="list-check-circle primary">
                                <li>Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry.</li>
                                <li>Dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy</li>
                                <li>Printer took a galley of type and scrambled it to make a type specimen book. </li>
                                <li>Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard.</li>
                                <li>Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the dummy book. </li>
                                <li>Eorem ipsum dolor sit amete printing and typesetting industry. </li>
                            </ul>
                            <h2 className="m-t30 m-b10 ">Health Care</h2>
                            <p className="m-b20">Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy
                                text ever since the  when an unknown printer took a galley of type and scrambled it to make a type specimen book. Lorem Ipsum is simply
                                dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the  when an
                                unknown printer took a galley of type and scrambled it to make a type specimen book. </p>
                            <p>Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been
                                the industry's standard dummy text ever since the  when an unknown printer took a galley of type and
                                scrambled it to make a type specimen book. </p>
                            <h3 className="m-b10">Our Help </h3>
                            <ul className="list-check-circle primary">
                                <li>Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry.</li>
                                <li>Dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy</li>
                                <li>Printer took a galley of type and scrambled it to make a type specimen book. </li>
                                <li>Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard.</li>
                                <li>Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the dummy book. </li>
                                <li>Eorem ipsum dolor sit amete printing and typesetting industry. </li>
                            </ul>
                        </div>

                        <div className="col-lg-3">
                            <div className="row">
                                <div className="col-lg-12 col-sm-6">
                                    <div className="icon-bx-wraper bx-style-1 p-a30 center m-b30">
                                        <div className="icon-bx-sm text-primary bg-white radius border-2 m-b20"> <a href="#" className="icon-cell"><i className="fa fa-user"></i></a> </div>
                                        <div className="icon-content">
                                            <h5 className="dez-tilte text-uppercase">Recent Highlights</h5>
                                            <p>Lorem ipsum dolor sit amet, consectetuer adipiscing elit, sed diam nonummy nibh euismod. </p>
                                        </div>
                                    </div>
                                </div>
                                <div className="col-lg-12 col-sm-6">
                                    <div className="icon-bx-wraper bx-style-1 p-a30 center m-b30">
                                        <div className="icon-bx-sm text-primary bg-white radius border-2 m-b20"> <a href="#" className="icon-cell"><i className="fa fa-building-o"></i></a> </div>
                                        <div className="icon-content">
                                            <h5 className="dez-tilte text-uppercase">Recent Highlights</h5>
                                            <p>Lorem ipsum dolor sit amet, consectetuer adipiscing elit, sed diam nonummy nibh euismod. </p>
                                        </div>
                                    </div>
                                </div>
                                <div className="col-lg-12 col-sm-12">
                                    <div className="icon-bx-wraper bx-style-1 p-a20 left m-b30">
                                        <div className="icon-bx-xs text-primary bg-white radius border-2 "> <a href="#" className="icon-cell"><i className="fa fa-code"></i></a> </div>
                                        <div className="icon-content">
                                            <h5 className="dez-tilte text-uppercase">Content title</h5>
                                            <p>Lorem ipsum dolor sit elit nonummy dolor is euismod end </p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </>
    )
}

export default Help