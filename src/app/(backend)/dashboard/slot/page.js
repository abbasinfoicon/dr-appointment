...
const Slot = () => {
    ...
    const columns = useMemo(
        () => [
            ...
            },
            {
                ...
                Cell: ({ row }) => (
                    <div>
                        <Link href={`slider/${row.original.id}`} className='btn rounded btn-primary'><i className="icon-eye"></i></Link>
                        <Link href={`slider/edit/${row.original.id}`} className='btn rounded btn-info mx-1'><i className="icon-pencil"></i></Link>
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const res = await FetchData({ url: "app/slot_c", method: "GET" });
            ...
            <div className="row page-titles mx-0">
                ...
                <div className="col-sm-6 justify-content-sm-end mt-2 mt-sm-0 d-flex">
                    <ol className="breadcrumb">
                        <li className="breadcrumb-item"><Link href="/dashboard/slot">Slots</Link></li>
                    ...
                    <div className="card">
                        <div className="card-header">
                            ...
                            <div className="add-new text-right">
                                <Link href="/dashboard/slot/add" className="btn btn-outline-primary btn-lg btn-rounded mt-1 pl-5 pr-5 add-new">Add New</Link>
                            </div>
                        </div>
